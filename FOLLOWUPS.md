...
| P4 | ✅ Done | `/manage-project init` Zotero collection 자동 생성 옵션 | 2026-04-24: `scripts/init_project.py`에 `--zotero-collection NAME` 추가. pyzotero + env vars(`ZOTERO_API_KEY`/`ZOTERO_LIBRARY_ID`/`ZOTERO_LIBRARY_TYPE`) 로 collection 생성 후 SSOT/legacy contract의 `library_id`/`collection_key` 자동 채움. 의존성/자격 없을 시 graceful degrade(WARN + null 유지). SKILL.md 문서 업데이트. |
...
| P8 | ✅ Done | `/lit-sync` Phase 2.5 polling 회귀 스크립트 추출 | 2026-04-24: `skills/lit-sync/tests/test_poll_logic.sh` 생성. 4 시나리오(detect-within-window / timeout-silent / debounce-late / slow-bbt-timeout). Self-contained: inline poll script + tmpdir fixture, macOS `stat -f`/Linux `stat -c` 양쪽 지원. 로컬 4/4 PASS. |
