...
Pipeline:
  Table 1 demographics -> weighted prevalence -> logistic regression ->
  subgroup analysis -> figures

Usage: python3 02_analyze.py
Output: output/table1.csv, output/regression_results.csv, figures/*.png
"""
...
# Exclude underweight (n=74) for cleaner analysis
...
for g in groups:
    gdf = group_dfs[g]
    # Weighted prevalence (point estimate)
    w = gdf["survey_weight"]
    weighted_prev = np.average(gdf["diabetes"], weights=w)
    # Unweighted prevalence + Wilson CI (consistent pair)
    ...
    prev_rows.append({
        "BMI Category": g,
        "n": n_total,
        "n diabetes": n_diabetes,
        "Prevalence (%)": f"{100*unweighted_prev:.1f}",
        "95% CI lower": f"{100*lo:.1f}",
        "95% CI upper": f"{100*hi:.1f}",
...
prev_df.to_csv("output/prevalence_by_bmi.csv", index=False)
print("\n")
print(prev_df.to_markdown(index=False))
print(f"\nSaved: output/prevalence_by_bmi.csv")

# ============================================================
# PART C: LOGISTIC REGRESSION — WEIGHTED
# ============================================================
print("\n" + "=" * 60)
...
# Education dummies (reference: College graduate+)
...
# Normalize survey weights so sum = n (preserves point estimates, gives
# realistic CIs based on actual sample size, not inflated pseudo-population)
...
print("\n--- Model 1: Unadjusted (survey-weight normalized) ---\n")
print(result1.summary2().tables[1].to_string())

# Model 2: Adjusted (age, sex, race, education)
...
or_combined.to_csv("output/regression_results.csv", index=False)
print(f"\nSaved: output/regression_results.csv")
...
# Figure 1: Diabetes prevalence by BMI category (bar chart)
# Use unweighted prevalence with Wilson CI for consistent error bars
...
ax.set_title("Figure 1. Diabetes Prevalence by BMI Category\n(NHANES 2017-2018, unweighted with Wilson 95% CI)")
...
plt.close(fig)
print("Saved: figures/prevalence_by_bmi.png")

# Figure 2: OR forest plot
fig, ax = plt.subplots(figsize=(6, 4))

# Plot adjusted ORs for key variables
plot_vars = or_table2[or_table2["Model"] == "Adjusted"][["Variable", "OR", "95% CI"]].copy()
plot_vars = plot_vars.iloc[::-1].reset_index(drop=True)  # reverse for forest plot

for i, row in plot_vars.iterrows():
    or_val = float(row["OR"])
    ci_str = row["95% CI"].split("-")
    ci_lo = float(ci_str[0])
    ci_hi = float(ci_str[1])
    ...
    ax.text(max(ci_hi, or_val) + 0.1, i,
            f"{or_val} ({ci_lo:.2f}-{ci_hi:.2f})",
...
ax.set_title("Figure 2. Adjusted Odds Ratios for Diabetes\n(NHANES 2017-2018, survey-weighted)")
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
ax.set_xlim(left=0)
...
plt.close(fig)
print("Saved: figures/or_forest_plot.png")
...
ax.set_title("Figure 3. HbA1c Distribution by BMI Category\n(NHANES 2017-2018)")
...
plt.close(fig)
print("Saved: figures/hba1c_distribution.png")

# Figure 4: Prevalence by age group and BMI category
fig, ax = plt.subplots(figsize=(6, 4))
...
ax.set_ylabel("Diabetes Prevalence (%)")
ax.set_title("Figure 4. Diabetes Prevalence by Age and BMI\n(NHANES 2017-2018)")
...
plt.close(fig)
print("Saved: figures/prevalence_by_age_bmi.png")

# ============================================================
# PART E: MANUSCRIPT-READY RESULTS TEXT
# ============================================================
print("\n" + "=" * 60)
...
n_total = len(df_main)
n_normal = len(group_dfs["Normal"])
n_overweight = len(group_dfs["Overweight"])
n_obese = len(group_dfs["Obese"])
n_diabetes = df_main["diabetes"].sum()

print(f"A total of {n_total} adults aged 20 years and older from NHANES 2017-2018 were included, "
      f"of whom {n_normal} ({100*n_normal/n_total:.1f}%) had normal BMI, "
      f"{n_overweight} ({100*n_overweight/n_total:.1f}%) were overweight, "
      f"and {n_obese} ({100*n_obese/n_total:.1f}%) were obese (Table 1).")
print()

# Get prevalence values
for g in categories:
    row = prev_df[prev_df["BMI Category"] == g]
    print(f"Diabetes prevalence was {row['Prevalence (%)'].values[0]}% "
          f"(95% CI: {row['95% CI lower'].values[0]}%-{row['95% CI upper'].values[0]}%) "
          f"in the {g.lower()} group.")
print()
...
print(f"In adjusted logistic regression, obesity was significantly associated with diabetes "
      f"(OR {ob_row['OR']}, 95% CI: {ob_row['95% CI']}, p {ob_row['p-value']}). "
      f"Overweight was also associated with increased odds "
      f"(OR {ow_row['OR']}, 95% CI: {ow_row['95% CI']}, p {ow_row['p-value']}) (Figure 2).")
print()
print("=" * 60)
...
Study type: Cross-sectional epidemiological study (NHANES)

## Tables
- `output/table1.csv` -- Baseline characteristics by BMI category
- `output/prevalence_by_bmi.csv` -- Survey-weighted diabetes prevalence
- `output/regression_results.csv` -- Logistic regression ORs (unadjusted + adjusted)

## Figures
- `figures/prevalence_by_bmi.pdf` -- Diabetes prevalence bar chart
- `figures/or_forest_plot.pdf` -- Adjusted OR forest plot
- `figures/hba1c_distribution.pdf` -- HbA1c distribution by BMI
- `figures/prevalence_by_age_bmi.pdf` -- Prevalence by age and BMI subgroup

## Data
- `data/nhanes_2017_2018.csv` -- Prepared NHANES dataset (n=4,940)
- `data/DEMO_J.XPT` -- Raw demographics
- `data/BMX_J.XPT` -- Raw body measures
- `data/GHB_J.XPT` -- Raw glycohemoglobin
"""
